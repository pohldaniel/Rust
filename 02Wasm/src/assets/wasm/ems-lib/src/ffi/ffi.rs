...
extern "C" {
	pub type GLFWerrorfun = fn(c_int, *const c_char);
	pub type GLFWwindowposfun = fn(*mut GLFWwindow, c_int, c_int);
	pub type GLFWwindowsizefun = fn(*mut GLFWwindow, c_int, c_int);
	pub type GLFWwindowclosefun = fn(*mut GLFWwindow);
	pub type GLFWwindowrefreshfun = fn(*mut GLFWwindow);
	pub type GLFWwindowfocusfun = fn(*mut GLFWwindow, c_int);
	pub type GLFWwindowiconifyfun = fn(*mut GLFWwindow, c_int);
	pub type GLFWframebuffersizefun = fn(*mut GLFWwindow, c_int, c_int);
	pub type GLFWmousebuttonfun = fn(*mut GLFWwindow, c_int, c_int, c_int);
	pub type GLFWcursorposfun = fn(*mut GLFWwindow, c_double, c_double);
	pub type GLFWcursorenterfun = fn(*mut GLFWwindow, c_int);
	pub type GLFWscrollfun = fn(*mut GLFWwindow, c_double, c_double);
	pub type GLFWkeyfun = fn(*mut GLFWwindow, c_int, c_int, c_int, c_int);
	pub type GLFWcharfun = fn(*mut GLFWwindow, c_uint);
	pub type GLFWcharmodsfun = fn(*mut GLFWwindow, c_uint, c_int); // TODO: Not yet exposed
	pub type GLFWdropfun = fn(*mut GLFWwindow, c_int, *mut *const c_char); // TODO: Not yet exposed
	pub type GLFWmonitorfun = fn(*mut GLFWmonitor, c_int);
	pub type GLFWjoystickfun = fn(c_int, c_int);
	pub type GLFWwindowmaximizefun = fn(*mut GLFWwindow, c_int);
	pub type GLFWwindowcontentscalefun = fn(*mut GLFWwindow, c_float, c_float);
}
