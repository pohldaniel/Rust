...
fn format_url(file_name: &str) -> reqwest::Url {
    ...
    let base = reqwest::Url::parse(&format!(
        ...
        option_env!("RES_PATH").unwrap_or("Rust/assets/res"),
    )).unwrap();
...
pub async fn load_model(
    ...
    for m in obj_materials? {
        let diffuse_texture = load_texture(&m.diffuse_texture.as_deref().unwrap(), false, device, queue).await?;
        let normal_texture = load_texture(&m.normal_texture.as_deref().unwrap(), true, device, queue).await?;
